...
export default function ManagePage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setJoinMsg({ ok: false, text: e instanceof Error ? e.message : "Could not join." });
    }
  };

  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-8">
      <h1 className="font-serif text-3xl font-semibold text-ink">Manage quotebooks</h1>
      <p className="mt-1 text-sm text-ink-muted">
        Generate invite codes, admit new members, and tidy up your spaces.
      </p>

      {!isSupabaseConfigured && (
        <div className="qb-card mt-6 border-amber-200 bg-amber-50/60 p-4 text-sm text-amber-800">
      ...
      <section className="mt-8">
        ...
        {joinMsg && (
          <p className={`mt-2 text-sm ${joinMsg.ok ? "text-emerald-700" : "text-red-600"}`}>
            {joinMsg.text}
          </p>
        )}
      </section>
...
function ManageBookRow({
  ...
  return (
    <div className="qb-card overflow-hidden">
      <button onClick={toggle} className="flex w-full items-center justify-between px-4 py-3 text-left hover:bg-black/[0.02]">
        ...
        <div className="border-t border-black/[0.06] px-4 py-4">
          ...
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <button
              ...
              className="qb-btn-ghost border border-black/10"
            >
              Rename
            </button>
          </div>
                    ...
                    return (
                      <li key={inv.id} className="flex items-center justify-between rounded-lg bg-paper px-3 py-2">
                        <div>
                          ...
                          <span className={`ml-2 text-xs ${expired ? "text-red-500" : "text-ink-muted"}`}>
                            {expired ? "expired" : `expires ${new Date(inv.expires_at).toLocaleString()}`}
                          </span>
                        </div>
                        <div className="flex items-center gap-1">
                          <button onClick={() => handleCopy(inv.code)} className="rounded-md px-2 py-1 text-xs hover:bg-black/5">
                            {copied === inv.code ? "Copied!" : "Copy"}
                          </button>
                          <button onClick={() => { void revokeInvite(inv.id); setInvites((p) => p.filter((x) => x.id !== inv.id)); }} className="rounded-md px-2 py-1 text-xs text-red-600 hover:bg-red-50">
                            Revoke
                          </button>
                        </div>
                      </li>
